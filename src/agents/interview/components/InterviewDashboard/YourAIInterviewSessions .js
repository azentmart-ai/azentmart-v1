...
const YourAIInterviewSessions = ({ setActivePage }) => {
  ...
  const loadUserSessions = async () => {
    ...
    if (!user?.id) return;
    try {
      const res = await fetch(`${process.env.REACT_APP_INTERVIEW_API_BASE_URL || "http://localhost:8000"}/api/interviews/user/${user.id}`);
  ...
  const loadAvailableDocuments = async () => {
    ...
    try {
      const user = getUser();
      if (!user?.id) return;
      const res = await fetch(`${process.env.REACT_APP_INTERVIEW_API_BASE_URL || "http://localhost:8000"}/api/documents/user/${user.id}`);
      const data = await res.json();
